...
Firstly - a caveat: the sprints that BeeWare (and to a lesser extent, [Django](https://djangoproject.com)) have run have been very focussed on attracting first-time Open Source contributors and building buzz about the project. That’s not the only way to run a sprint, and it’s not the only audience to attract. If your goals are different, you might not gain as much from this guide.
